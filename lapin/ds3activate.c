...
static void activate_gamepad(void *device)
{
    ...
    if (r != kIOReturnSuccess) {
        cLog("  Failed to open device - %d", r);
        return;
    }
    ...
    cLog("  Should be rumbling!");
} /* activate_gamepad */


void ds3activate_main(void)
{
    ...
    if (deviceSet != NULL) {
        ...
        if (count > 0) {
            cLog("Discovered %ld DualShock 3 gamepads", count);
            void **gamepads = calloc(count, sizeof(void *));
            CFSetGetValues(deviceSet, (const void **)gamepads);
            for (CFIndex i = 0; i < count; i++) {
                cLog("Handling device %ld:", i);
                activate_gamepad(gamepads[i]);
            }
            free(gamepads);
        } else {
            cLog("No DualShock 3 gamepads found!");
        }
        CFRelease(deviceSet);
    } else {
        cLog("No usb %04X/%04X found!",  VendorId ,ProductId);
